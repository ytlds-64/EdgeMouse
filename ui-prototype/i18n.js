(() => {
  const zhToEn = {
    "像系统显示设置一样拖动屏幕，贴合的边缘自动成为穿越区域": "Drag screens as in system display settings; touching edges become crossing regions",
    "拖动屏幕贴合边缘，松手后自动保存 · 方向键也可微调": "Drag screens to join edges; release to save · Arrow keys adjust positions",
    "排列屏幕": "Arrange displays",
    "自动排列": "Arrange automatically",
    "连接两台电脑后可拖动屏幕": "Connect both computers to arrange displays",
    "屏幕不能重叠，请移到相邻边缘": "Screens cannot overlap. Move to an adjacent edge",
    "屏幕已变化，请重新排列并确认穿越边缘": "Displays changed. Arrange them again and check the crossing edges",
    "高亮边缘可以穿越，位置重叠的区域自动对应": "Highlighted edges allow crossing; overlapping edge regions map automatically",
    "屏幕之间有空隙，将边缘贴合即可穿越": "There is a gap between screens. Join their edges to enable crossing",
    "拖动结束后自动保存并同步到另一台电脑": "Positions save and sync automatically when dragging ends",

    "屏幕连接配置读取失败，请重试": "Unable to read screen connections. Please retry",
    "两端均需支持独立屏幕连接": "Both computers need support for individual display connections",
    "包含缩放、旋转和多显示器 · 已启用": "Scaling, rotation and multiple displays · Enabled",
    "选择参与跨屏的显示器，穿越区域按屏幕布局自动分配": "Choose displays for sharing; crossing regions follow the screen layout automatically",
    "点击屏幕勾选或取消，可同时连接多块屏幕": "Click a screen to select or deselect it; multiple displays can be connected",
    "选择屏幕后自动保存，连接后同步到另一台电脑": "Screen selections save automatically and sync when connected",
    "参与跨屏的屏幕": "Displays for screen sharing",
    "连接全部屏幕": "Connect all displays",
    "连接两台电脑后可选择屏幕": "Connect both computers to choose displays",
    "屏幕排列已变化，请重新选择屏幕": "Display layout changed. Please select your displays again",
    "当前没有可穿越边缘，请选择屏幕或调整方向": "No crossing edges. Select displays or change the direction",
    "已选屏幕按位置和边缘长度自动分配": "Selected displays share the edge by position and edge length",
    ...
    "展开侧栏": "Expand sidebar",
    "保存后自动同步到另一台电脑": "Automatically sync to the other computer after saving",
    "任意一端修改并保存即可同步；两个控制方向独立保存": "Edit and save on either device to sync; each control direction is saved independently",
    "设置已保存在本机，等待另一端同步（两端需更新到 0.6.6 或更高版本）": "Saved locally, waiting to sync (both devices need 0.6.6 or later)",
    "输入设置已同步；在任意一端修改并保存即可": "Input settings synced; edit and save on either device",
    "主导航": "Main navigation",
    ...
    "配置状态": "Configuration",
    "两端一致": "Synchronized",
    "正在同步布局": "Synchronizing layout",
    "等待连接后自动同步": "Will synchronize automatically when connected",
    "已连接 · 布局已保存": "Connected · Layout saved",
    "布局已保存，正在等待另一端确认": "Layout saved; waiting for the other device to confirm",
    "布局已保存；在任意一端修改并保存即可自动同步": "Layout saved; edit and save on either device to synchronize automatically",
    "布局已保存；下次连接后将自动同步到另一台电脑，无需再次保存": "Layout saved; it will synchronize on the next connection without saving again",
    "拖动或选择方向后保存，两端会自动同步": "Drag or choose a direction, then save to synchronize both devices",
    "布局已修改，点击保存后会同步到两台设备": "Layout changed; save to synchronize both devices",
    "尚未保存": "Not saved",
    "正在同步": "Synchronizing",
    "等待连接同步": "Waiting for a connection to synchronize",
    "布局已保存，两端配置一致": "Layout saved; both devices are synchronized",
    "布局已保存，两端将自动重新连接": "Layout saved; both devices will reconnect automatically",
    "布局已保存，正在同步并重新连接": "Layout saved; synchronizing and reconnecting",
    "布局已保存；后台服务未运行，启动服务后请再点一次保存以同步对端": "Layout saved; start the background service and save once more to synchronize the peer",
    "保存布局": "Save layout",
    ...
    "鼠标移动速度": "Pointer speed",
    "设置已保存在本机，等待另一端同步（鼠标速度同步需两端 0.6.9 或更高版本）": "Saved locally, waiting for peer sync (pointer speed requires 0.6.9+ on both computers)",
    "仅调整当前方向的远程移动，100% 为默认速度": "Remote movement in this direction only; 100% is the default",
    ...
    "本机鼠标优先": "Local mouse priority",
    "移动本机鼠标，立即恢复本机控制": "Move this computer's mouse to restore local control immediately",
    ...
    "此项由另一台电脑负责，请在另一端的 EdgeMouse 中设置": "This setting belongs to the other computer; configure it in EdgeMouse on that device",
    "指针平滑由被控制的电脑负责，请在另一端设置": "Pointer smoothing belongs to the controlled computer; configure it on the other device",
    "本页可保存这台电脑发送的滚轮、键盘与拖拽设置；指针平滑和抢回请在另一端设置": "This page saves wheel, keyboard, and drag settings sent by this computer. Configure pointer smoothing and reclaim on the other device",
    "本页可保存这台电脑接收的指针平滑与抢回设置；滚轮和键盘请在另一端设置": "This page saves pointer smoothing and reclaim settings received by this computer. Configure wheel and keyboard settings on the other device",
    ...
    "连接、断开和自动重连时提醒": "Notify on connect, disconnect, and automatic reconnection",
    "外观": "Appearance",
    ...
    "以前已经配对过？": "Already paired before?",
    "选择旧版使用的 edgemouse.toml，可直接沿用原设备身份和可信证书。": "Select the edgemouse.toml used by an earlier version to keep the existing device identity and trusted certificate.",
    "导入旧配对配置…": "Import previous pairing…",
    ...
    "已打开诊断包所在位置": "Diagnostics package shown in its folder",
    "请先生成诊断包": "Generate the diagnostics package first",
